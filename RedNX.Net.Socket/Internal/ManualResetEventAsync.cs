﻿// Copyright (c) badflyer.com. All rights reserved.

using System;
using System.Threading;
using System.Threading.Tasks;

namespace RedNX.Net.Socket.Internal {

    /// <summary>
    /// An async manual reset event.
    /// </summary>
    internal sealed class ManualResetEventAsync {
        ...
        private async Task<bool> AwaitCompletion(int timeoutMs, CancellationToken token) {
            // Validate arguments.
            if (timeoutMs < -1) {
                throw new ArgumentException("The timeout must be either -1ms (indefinitely)");
            }
            ...
            var delayTask = Task.Delay(timeoutMs, timeoutToken.Token).ContinueWith((result) => { var unused = result.Exception; }, TaskContinuationOptions.ExecuteSynchronously);
            using (timeoutToken) {
